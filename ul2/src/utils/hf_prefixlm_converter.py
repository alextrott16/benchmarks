...
def convert_bloom_causal_lm_to_prefix_lm(model: BloomForCausalLM) -> BloomForCausalLM:
...
def convert_opt_causal_lm_to_prefix_lm(model: OPTForCausalLM) -> OPTForCausalLM:
...
def convert_hf_causal_lm_to_prefix_lm(model: CAUSAL_LM_TYPES) -> CAUSAL_LM_TYPES:
    ...
    if isinstance(model, _SUPPORTED_GPT_MODELS):
        return convert_gpt_causal_lm_to_prefix_lm(model)

    elif isinstance(model, BloomForCausalLM):
        return convert_bloom_causal_lm_to_prefix_lm(model)

    elif isinstance(model, OPTForCausalLM):
        return convert_opt_causal_lm_to_prefix_lm(model)
