...
"""
Perform lightweight surgery on a HuggingFace Causal LM to convert it to a Prefix LM that accepts a `bidirectional_mask` input.
"""
...
from transformers.models.bloom.modeling_bloom import _make_causal_mask as _make_causal_mask_bloom, _expand_mask as _expand_mask_bloom
from transformers.models.opt.modeling_opt import OPTForCausalLM
from transformers.models.opt.modeling_opt import _make_causal_mask as _make_causal_mask_opt, _expand_mask as _expand_mask_opt
...
def _convert_bloom_causal_lm_to_prefix_lm(model: BloomForCausalLM) -> BloomForCausalLM:
    ...
    def forward(
        ...
        bidirectional_mask: Optional[torch.Tensor] = None, # <--- WE'RE ADDING A NEW ARGUMENT!
    ...
    def forward(
        ...
        past_key_values: Optional[Tuple[Tuple[torch.Tensor, torch.Tensor], ...]] = None,
        attention_mask: Optional[torch.Tensor] = None,
        bidirectional_mask: Optional[torch.Tensor] = None, # <--- WE'RE ADDING A NEW ARGUMENT! (Change 1/2)
        ...
        **deprecated_arguments
        ...
        transformer_outputs = self.transformer(
            ...
            bidirectional_mask=bidirectional_mask, # <--- WE'RE ADDING A NEW ARGUMENT! (Change 2/2)
            head_mask=head_mask,
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs
        ...
        return {
            ...
            "use_cache": True, # Requires this. TODO(Alex): Confirm this supports other decoding strategies.
            "attention_mask": attention_mask,
...
def _convert_opt_causal_lm_to_prefix_lm(model: OPTForCausalLM) -> OPTForCausalLM:
    ...
    def forward(self: OPTForCausalLM, *args: tuple, bidirectional_mask: Optional[torch.ByteTensor]=None, **kwargs: Dict[str, Any]):
        if bidirectional_mask is None:
            # This wrapper is a no-op if bidirectional masks are not supplied
            return self._original_forward(*args, **kwargs) # type: ignore
...
def convert_hf_causal_lm_to_prefix_lm(model: CAUSAL_LM_TYPES) -> CAUSAL_LM_TYPES:
    ...
    if not isinstance(model, _SUPPORTED_HF_MODELS):
        raise TypeError(f'Cannot convert model to Prefix LM. Model does not belong to set of supported HF models:\n{_SUPPORTED_HF_MODELS}')
